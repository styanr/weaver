<script lang="ts">
	const { pageSize, currentPage, totalCount, onChange, class: className = '' } = $props();
...
<div class={`${className} grid grid-cols-3`}>
	{#if hasPrevious}
		<div class="col-start-1">
			<button class="btn w-fit text-xl btn-soft" onclick={() => onChange(previousPage)}>
				{previousPage}
			</button>
		</div>
	{/if}
	...
	{#if hasNext}
		<div class="col-start-3 flex flex-row justify-end">
			<button class="btn w-fit text-xl btn-soft" onclick={() => onChange(nextPage)}>
				{nextPage}
			</button>
		</div>
	{/if}
